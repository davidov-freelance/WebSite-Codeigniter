...
(function($, window, document){
  ...
  $(function(){
    ...
    var my_datatable = $('[data-table="true"]').dataTable({
    ...
    $('[data-type="datatable_select"]').change(function () {
          my_datatable.fnFilter($(this).find("option:selected").text(), $(this).attr("data-td-num"));
    });
    ...
    columnInputs.keyup(function () {
          my_datatable.fnFilter(this.value, columnInputs.index(this));
    });
    ...
    var dtInstance2 = $('#datatable2').dataTable({
        ...
        // Note the required keywords between underscores (e.g _MENU_)
        oLanguage: {
            sSearch:      'Поиск по данным статистики',
            sLengthMenu:  '_MENU_ строк в таблице',
            info:         'Показано _PAGE_ из _PAGES_',
            zeroRecords:  'Ничего не найдено',
            infoEmpty:    'Нет данных',
            infoFiltered: '(filtered from _MAX_ total records)'
...
(function($, window, document){
  ...
  $(window).load(function() {
    ...
    $.fn.slimScroll && $('.scroll-content').slimScroll({
        height: '250px'
    });
  ...
  $(function() {
    ...
    // Filestyle
    $.fn.filestyle && $('.filestyle').filestyle();
    ...
    $("#activatedCollapse").click(function(){
        isCollapsed = $("#isCollapsed").hasClass("aside-collapsed");
        if(isCollapsed)
            $("#isCollapsed").removeClass("aside-collapsed");
        else
            $("#isCollapsed").addClass("aside-collapsed");
        $.post(url + "home/changeCollapsed", {isCollapsed: !isCollapsed});
    });
