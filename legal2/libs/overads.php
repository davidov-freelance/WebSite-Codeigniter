<?php
class Overads {
	private $api_url = "http://overads.dev/api/";
	public $script_url = "http://overads.dev/scripts/";
	private $api_key = "crqh680ecwoks0w4wko4kcowg0c080k8";
	// этот ключ используеются при прямом переходе пользователя
	private $flow_key = "PuNfy";
	public $hash;
	public $goalId;
	public $term_group;
	public $data;
	...
	public function __construct( $goalID, $term_group = 0 ){
		if( isset( $_GET['hash'] ) ){
			...
			$this->hash = $this->sendRequest("transit", $this->flow_key );
		}
		$this->goalId = $goalID;
